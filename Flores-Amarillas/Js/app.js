...
document.addEventListener('DOMContentLoaded', function() {
    ...
    CrearVarias(); // Llamar a CrearVarias directamente
...
function DibujarFlor(x, y, NumeroPetalos, RadioXPetalo, RadioYPetalo, AltoTrazo) {
    ...
    function dibujarSiguientePetalo() {
        ...
        ctx.beginPath();
        ctx.arc(x, y, 40, 0, Math.PI * 2); // Centro mucho más grande
...
function CrearVarias() {
    const numFlores = 30; // Cambiar a 30 flores

    // Espaciamiento y tamaño de cada flor
    const espacioX = canvas.width / 6; // Ajustar el espaciamiento según el número de flores
    const espacioY = canvas.height / 5; // Ajustar el espaciamiento según el número de flores
    const TamañoFlor = 130;

    for (let i = 0; i < numFlores; i++) {
        const fila = Math.floor(i / 6); // Ajustar el número de columnas
        const columna = i % 6; // Ajustar el número de columnas
        const x = espacioX * columna + espacioX / 2;
        const y = espacioY * fila + espacioY / 2;

        DibujarFlorSinTallo(x, y, 20, 30, 80, TamañoFlor); // Más pétalos
    }
}
